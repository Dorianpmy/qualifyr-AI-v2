...
export async function getEmailChannelSettings(slug: string) {
  ...
  if (error) throw new Error("email_channel_unavailable");
  const domain = getServerEnv().INBOUND_EMAIL_DOMAIN;
  return { organization, channel: data, address: data && domain ? `${data.route_key}@${domain}` : null, providerConfigured: Boolean(domain && getServerEnv().RESEND_API_KEY && getServerEnv().RESEND_WEBHOOK_SECRET) };
}
