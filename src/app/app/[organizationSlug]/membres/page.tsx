...
import { listOrganizationInvitations, listOrganizationMembers, listOrganizationsForUser, requireOrganizationMembership } from "@/server/organizations/service";
...
export default async function MembersPage({ params }: { params: Promise<{ organizationSlug: string }> }) {
  ...
  const [organization, organizations] = await Promise.all([
    requireOrganizationMembership(organizationSlug), listOrganizationsForUser(),
  ]);
