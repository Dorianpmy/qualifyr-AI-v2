...
export function chooseNextQuestion(input:{playbook:PlaybookSchema;facts:FactForQuestion[];serviceUncertain:boolean;receivedProofKeys?:string[]}){
  ...
  const receivedProofs=new Set(input.receivedProofKeys??[]);const missingProof=input.playbook.proofs.find(proof=>!receivedProofs.has(proof.key));
  if(missingProof)return `Pouvez-vous fournir : ${missingProof.label.toLowerCase()} ?`;
