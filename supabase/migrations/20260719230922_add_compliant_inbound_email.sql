...
begin
  ...
  loop attempt:=attempt+1; new_reference:='D-'||upper(substr(pg_catalog.encode(extensions.gen_random_bytes(8),'hex'),1,10)); exit when not exists(select 1 from public.service_requests where organization_id=channel.organization_id and reference_code=new_reference); if attempt>=5 then raise exception 'reference_generation_failed'; end if; end loop;
