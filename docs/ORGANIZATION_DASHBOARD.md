...
La sidebar desktop et le drawer mobile exposent seulement les routes réellement disponibles : Accueil et Membres. La topbar contient le sélecteur d’organisation, le thème et la déconnexion. Aucune route vide n’est créée pour les fonctions futures.

## Service et DTO

`getOrganizationDashboard` est l’unique agrégateur serveur. Son DTO contient les paramètres de l’organisation, le rôle et le prénom de l’utilisateur courant, les compteurs d’équipe autorisés et quatre aperçus minimisés. Il ne contient ni email de membre, ni token, ni hash, ni invitation détaillée, ni objet Supabase brut.
...
Aucune métrique métier, activité inventée, graphique, Dossier ou modèle Phase 6 n’est présent. La carte « Bientôt disponible » est informative et non cliquable. La modification du profil et des paramètres d’organisation reste hors de cette phase ; leurs raccourcis ne sont donc pas simulés.
